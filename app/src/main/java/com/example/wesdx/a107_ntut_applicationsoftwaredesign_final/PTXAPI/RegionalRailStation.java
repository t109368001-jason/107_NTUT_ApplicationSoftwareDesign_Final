...
public class RegionalRailStation  implements Comparable<RegionalRailStation> {
    ...
    public static List<RegionalRailStation> convert(List<RailStation> railStationList) {
        ...
        for(int i = 0; i < railStationList.size(); i++) {
            boolean found = false;
            String regionName = "";
            RailStation railStation = railStationList.get(i);
            ...
            for(int j = 0; j < regionalRailStationList.size(); j++) {
                if(regionalRailStationList.get(j).regionName.compareTo(regionName) == 0) {
                    regionalRailStationList.get(j).railStationList.add(railStation);
                    found = true;
                }
            }

            if(!found) {
                RegionalRailStation regionalRailStation = new RegionalRailStation();
                regionalRailStation.regionName = regionName;
                regionalRailStation.railStationList.add(railStation);
                regionalRailStationList.add(regionalRailStation);
            }
