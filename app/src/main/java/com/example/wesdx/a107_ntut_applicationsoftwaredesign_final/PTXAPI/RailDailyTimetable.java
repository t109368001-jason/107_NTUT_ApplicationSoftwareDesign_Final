...
public class RailDailyTimetable {
    ...
    public static List<RailDailyTimetable> filterByOD(List<RailDailyTimetable> railDailyTimetableList, RailStation originStation, RailStation destinationStation, Date time, boolean isDirectional, boolean isOriginDepartureTime) throws ParseException {
        List<RailDailyTimetable> railDailyTimetableList_new = null;
        ...
        for(RailDailyTimetable railDailyTimetable:railDailyTimetableList) {
            ...
            for(int i = 0; i < railDailyTimetable.StopTimes.size(); i++) {
                if(railDailyTimetable.StopTimes.get(i).StationID.equals(destinationStation.StationID)) {
                    ...
                    if((API.timeFormat.parse(railDailyTimetable.StopTimes.get(i).DepartureTime).after(time)) && (!isOriginDepartureTime)) {
                        break;
                    }
                    findEnd = true;
                }
                if(railDailyTimetable.StopTimes.get(i).StationID.equals(originStation.StationID)) {
                    if((API.timeFormat.parse(railDailyTimetable.StopTimes.get(i).DepartureTime).before(time)) && (isOriginDepartureTime)) {
                        break;
                    }
                    findBeg = true;
                }
